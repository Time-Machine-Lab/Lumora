...
5. One timeout-aware request path serves connection tests, chat, and storyboard generation. It never reads a failed HTTP response body, never logs request data, never retries, and maps only status/signal/shape information to typed errors. A success-body JSON `SyntaxError` is `schema_invalid`; body stream, connection, and decode failures are retryable `network_error`. Reading the success body races the same composed signal as `fetch`, so caller cancellation, lifecycle cancellation, and deadline retain their stable codes after headers arrive and an abort-ignoring late body cannot win. The host replaces provider messages with fixed summaries at the task boundary.
...
8. Generic Chat and Storyboard model catalogs use one provider-neutral resolver with strict array, 1..100 item, safe iteration, entry, and unique-id validation. Any resolver or runtime-value failure becomes a host-owned `provider_unavailable`; Core still resolves immediately before dispatch and passes the already accepted `request.model` unchanged. Studio preserves errors created by the host error class and maps unknown thrown values to its fixed `invalid_request` fallback.

9. Generic Chat validates each runtime message before crossing the provider boundary and constructs a fresh object containing only `role` and `content`. The OpenAI-compatible transport repeats that projection at serialization so extra credential-shaped, cyclic, or `BigInt` fields cannot enter the request body even when the client is called directly.
