...
import type { AssetData, SceneEditor } from '@lumora/core';
 ...
 * 解析后的 GLTF 内容经 AssetCache 引用计数管理。
 */
export async function importModelFile(
  ...
  const latestProject = editor.getProject()!;
  const effective = findAssetByHash(latestProject, hash)!;
  cache.addRef(hash, created.value);
