...
时间线录制进行中不允许开始 PNG 或 WebM 画面导出，避免录制机位跳过轨道求值。编码按项目中的分镜顺序逐帧推进，源时间为 `shot.startTime + frameIndex / fps`。帧调度使用相对录制起点的绝对截止时间，避免把 WebGL 读回耗时累加到成片时长。固定 16:9 输出会在项目画幅外补背景，不拉伸 4:3 或竖屏项目。导出期间暂停时间线，完成、失败或取消后恢复原播放头；所有媒体轨道都会停止，临时画布和对象 URL 会释放。编辑网格、变换控件和 DOM 辅助线不进入 PNG 或视频帧。
