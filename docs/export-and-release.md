...
4. 浏览器提供 WebCodecs `VideoEncoder`、`VideoFrame`，并支持 VP8 WebM 编码。

时间线录制进行中不允许开始 PNG 或 WebM 画面导出，避免录制机位跳过轨道求值。编码按项目中的分镜顺序逐帧推进，源时间为 `shot.startTime + frameIndex / fps`。第 `i` 个编码帧显式使用 `round(i * 1,000,000 / fps)` 微秒 PTS，其 duration 为相邻量化时间戳之差，因此文件时钟不依赖墙钟、计时器节流或渲染耗时。`N` 个内容帧完成后，再把未改变的末帧编码到 `N / fps`，随后等待 `VideoEncoder.flush()`、完成 WebM muxer finalization，才允许下载。
...
WebM、PNG、清单和插件导出共享同一个工作台操作令牌，任一时刻只运行一项。关闭或卸载工作台、切换项目会话，或移除正在运行的插件导出器时，当前令牌立即失效；异步回调以及可重入的播放头跳转和画面捕获后必须重新验证令牌，因此过期操作不能继续捕获、下载文件、恢复旧播放头或覆盖新会话状态。导出期间暂停时间线，完成、失败或取消后恢复原播放头；所有路径都会关闭 `VideoEncoder`、缩小临时画布并释放对象 URL。
...
兼容测试使用 3 个持久化重开后仍视觉可区分的机位，验证 EBML 文件头、非零文件、目标分辨率、可解码画面、逐段画面顺序、packet/末 PTS/容器时长、取消不下载、编码器关闭和取消后的继续编辑。Safari 不能在 Windows CI 上作可信替代，必须在 macOS 真机人工签署降级结果。
...
同一文件还覆盖真实 WebM 播放/时长、14 帧/24fps 与 45 帧/30fps 产物探针、无 WebCodecs 时零 encoder/零下载、取消和会话失效后的 encoder 关闭、flush 失败/取消重试，以及返回编辑。测试收集 `pageerror` 与 error 级别控制台消息；主流程要求集合为空。
